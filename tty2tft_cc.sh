  ...
  if [ ${#} -gt 3 ]; then # We don't accept more than 2 parameters
    ...
    while [ ${#} -gt 0 ]; do
      case ${1,,} in
        ...
        disable)
          clear
          echo -e "${fred}Disable${freset} tty2tft at boot time"
        ...
        enable)
          clear
          echo -e "${fgreen}Enable${freset} tty2tft at boot time"
...
function tty_main() {
  ...
  for i in {10..1}; do
    echo -ne " Start tty2tft Update in ${i}...\033[0K\r"
    premenu="Update"
...
function tty_menu() {
  ...
  --backtitle "tty2tft" --title "[ Utilities ]" \
  --menu "Use the arrow keys and enter \nor the d-pad and A button" 0 0 0 \
  Ota "Start ESP OTA (ESP32 only)" \
  Reset "Reset ESP (ESP32 only)" \
  Stop "Stop tty2tft Daemon" \
  Start "Start tty2tft Daemon" \
  Restart "Restart tty2tft Daemon" \
  Disable "Disable tty2tft at boot" \
  Enable "Enable tty2tft at boot" \
  Slide_Menu "Picture Slideshow" \
  Update "Update tty2tft" \
...
function slide_menu() {
  ...
  dialog --clear --no-cancel --ascii-lines --no-tags \
  --backtitle "tty2tft" --title "[ Slideshow ]" \
  --menu "Use the arrow keys and enter \nor the d-pad and A button" 0 0 0 \
...
function waitforttyack() {
  echo -n "Waiting for tty2tft Acknowledge... "
